...
import com.smithies.angularreports.factory.InterlockingVOFactory;
...
public class InterlockingController {
    ...
    @GetMapping("/id/{id}")
    public @ResponseBody
    InterlockingVO findById(@PathVariable("id") Short id) {
        return InterlockingVOFactory.generateVO(repo.findOne(id));
    }

    @GetMapping("/search")
    public @ResponseBody
    List<InterlockingVO> search(@RequestParam("search") String siteName) {
        List<Interlocking> sites = new ArrayList<>();
    ...
    @GetMapping("/{name}/id")
    public @ResponseBody
    Short getIdForName(@PathVariable("name") String siteName) {
